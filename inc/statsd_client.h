...
class Event
{
  ...
  public:
    ...
    void set_aggregation_Key(std::string aggregation_Key)
    {
        aggregation_Key_ = aggregation_Key;
    }
    void set_priority(Priority priority)
    {
        priority_ = priority;
    }
    void set_source_type_name(std::string source_type_name)
    {
...
class StatsdClient
{
  ...
  public:
     ...
     * type = "c", "g" or "ms"
     */
    int send(std::string key, size_t value, const std::string& type, float sample_rate, tags_t tags);
  ...
  private:
    bool should_send(float sample_rate);
};
}; // namespace statsd
