...
struct StatsdClient::StatsdClientData
{
    int sock;
...
bool StatsdClient::should_send(float sample_rate)
{
...
int StatsdClient::dec(const std::string& key, float sample_rate, tags_t tags)
{
    return count(key, -1, sample_rate, tags);
}

int StatsdClient::inc(const std::string& key, float sample_rate, tags_t tags)
{
    return count(key, 1, sample_rate, tags);
}

int StatsdClient::count(const std::string& key, size_t value, float sample_rate, tags_t tags)
{
    return send(key, value, "c", sample_rate, tags);
}

int StatsdClient::gauge(const std::string& key, size_t value, float sample_rate, tags_t tags)
{
    return send(key, value, "g", sample_rate, tags);
}

int StatsdClient::timing(const std::string& key, size_t ms, float sample_rate, tags_t tags)
{
...
int StatsdClient::send(std::string key, size_t value, const std::string& type, float sample_rate, tags_t tags)
{
    ...
    if (fequal(sample_rate, 1.0))
    {
        snprintf(buf, sizeof(buf), "%s%s:%zd|%s%s", d->ns.c_str(), key.c_str(), value, type.c_str(), tags_str.c_str());
    }
    else
    {
        snprintf(buf, sizeof(buf), "%s%s:%zd|%s|@%.2f%s", d->ns.c_str(), key.c_str(), value, type.c_str(), sample_rate, tags_str.c_str());
    }
...
int StatsdClient::send(const std::string& message)
{
    ...
    if (ret == -1)
    {
        snprintf(d->errmsg, sizeof(d->errmsg), "sendto server fail, host=%s:%d, err=%m", d->host.c_str(), d->port);
        return -1;
    }
    return 0;
}
